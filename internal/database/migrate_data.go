...
// during `primevpn migrate-db --dsn` (SQLite → PostgreSQL data migration) and in
// related tests.
//
...
func MigrateData(srcPath, dstDSN string) error {
	...
	log.Println("Set XUI_DB_TYPE=postgres and XUI_DB_DSN=... in /etc/default/primevpn, then restart primevpn.")
	return nil
}
...
func PrepareSQLiteForMigration(dbPath string) error {
	...
	for _, table := range []string{"users", "settings", "inbounds"} {
		if !sqliteTableExists(sqlDB, table) {
			return fmt.Errorf("not a PRIMEVPN panel database: required table %q is missing", table)
		}
	}
	for _, m := range migrationModels() {
