...
PRIMEVPN 支持 TOTP 双因素认证（兼容 Google Authenticator、Aegis 等）。在**面板设置**中启用即可——启用后，登录页面除了密码外还会要求输入一个 6 位验证码，并且开启该功能会强制所有人重新登录。你可以通过菜单的 **Reset Username & Password** 步骤，或使用 `x-ui setting -resetTwoFactor` 来禁用它。
