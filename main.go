// Package main is the entry point for the PRIMEVPN web panel application.
...
// runWebServer initializes and starts the web server for the PRIMEVPN panel.
func runWebServer() {
...
// migrateDb performs database migration operations for the PRIMEVPN panel.
func migrateDb() {
...
// "primevpn setting" hit the same database backend as the panel. godotenv.Load does
...
// main is the entry point of the PRIMEVPN application.
...
func main() {
	...
	migrateDbCmd.StringVar(&migrateSrc, "src", "", "Source SQLite file (defaults to the configured primevpn.db)")
	...
	flag.Usage = func() {
		...
		fmt.Println("    migrate        migrate form other/old primevpn")
