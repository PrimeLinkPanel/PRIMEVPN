---
title: Security
description: Harden PRIMEVPN — panel auth and 2FA, custom-core IP limits, firewall rules, BBR, and staying current.
icon: ShieldCheck
---
...
IPs. PRIMEVPN's pinned custom Xray core rejects additional source IPs before
...
port, and your inbound ports. The `primevpn` menu (**22 — Firewall Management**) wraps
...
The `primevpn` menu (**25 — Enable BBR**) toggles Google's BBR congestion control
(`net.ipv4.tcp_congestion_control = bbr`, `net.core.default_qdisc = fq`), which
often improves throughput on congested links.

## Keep current

Update PRIMEVPN and Xray-core regularly — security fixes land in new releases. Watch
the [releases page](https://github.com/sh7CBAC/PRIMEVPN/releases) and see
