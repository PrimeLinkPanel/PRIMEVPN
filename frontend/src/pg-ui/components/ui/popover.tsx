...
import * as PopoverPrimitive from '@radiprimevpn/react-popover'
  ...
  ({ className, align = 'center', sideOffset = 4, collisionPadding = 8, disablePortal = false, ...props }, ref) => {
    const content = (
      <PopoverPrimitive.Content
        ...
        className={cn(
          'bg-popover text-popover-foreground data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 z-60 w-72 max-w-[calc(100vw-1rem)] origin-(--radix-popover-content-transform-origin) rounded-md border p-4 text-xs shadow-md outline-none',
          className,
        )}
        {...props}
      />
