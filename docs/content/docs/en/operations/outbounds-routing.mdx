...
description: Shape egress in PRIMEVPN — WARP and NordVPN outbounds, outbound subscriptions (server pools), routing rules, and load balancers.
icon: Route
---

Inbounds accept clients; **outbounds** decide where their traffic goes next.
PRIMEVPN can route traffic through Cloudflare WARP, NordVPN, or arbitrary outbound
...
shapes PRIMEVPN uses: **VLESS is the flat form** (`address`/`port`/`id`/`flow`/
...
WARP lets your server egress through Cloudflare's network. PRIMEVPN can register a
...
From the panel's WARP controls, register an account. PRIMEVPN fills the outbound's
...
PRIMEVPN can fetch NordVPN (NordLynx/WireGuard) credentials from an access token (or
