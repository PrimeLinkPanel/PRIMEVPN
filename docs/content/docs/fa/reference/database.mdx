---
title: پایگاه داده
description: بک‌اندهای ذخیره‌سازی PRIMEVPN — SQLite (پیش‌فرض) و PostgreSQL — مسیر پایگاه داده، استخر اتصال و مهاجرت از SQLite به PostgreSQL.
icon: Database
---

PRIMEVPN همه چیز را — inboundها، کلاینت‌ها و تنظیمات — در یک پایگاه داده ذخیره می‌کند.
بک‌اند را هنگام نصب انتخاب می‌کنید؛ هر دو کاملاً پشتیبانی می‌شوند.

## SQLite (پیش‌فرض)

یک فایل واحد در مسیر `/etc/primevpn/primevpn.db`. بدون نیاز به هیچ راه‌اندازی، ایده‌آل برای
...
`/etc/default/primevpn` می‌نویسد:

```bash title="/etc/default/primevpn"
...
سپس `XUI_DB_TYPE` و `XUI_DB_DSN` را در `/etc/default/primevpn` تنظیم کرده و سرویس را مجدداً
راه‌اندازی کنید:
