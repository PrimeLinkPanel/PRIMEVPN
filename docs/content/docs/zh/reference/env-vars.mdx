---
title: 环境变量
description: PRIMEVPN 的 XUI_* 环境变量完整参考——涵盖数据库、面板、日志、内存以及隧道健康监测器。
icon: Variable
---

PRIMEVPN 从 `XUI_*` 环境变量中读取其运行时配置。使用脚本安装时，安装程序会将
这些变量写入服务环境文件（`/etc/default/primevpn`，或视发行版而定的
`/etc/conf.d/primevpn` / `/etc/sysconfig/primevpn`）；使用 Docker 时，则在
`docker-compose.yml` 或 `docker run -e` 中设置。默认值已足够合理——只需设置
你需要更改的项，然后重启即可：`systemctl restart primevpn`。
...
| `XUI_DB_FOLDER`          | `/etc/primevpn` | SQLite 数据库文件（`primevpn.db`）所在的文件夹。                         |
...
默认的 SQLite 数据库路径为 `/etc/primevpn/primevpn.db`。有关 SQLite ↔ PostgreSQL 的
...
| `XUI_LOG_FOLDER` | `/var/log/primevpn`  | 日志输出目录。                                              |
...
| -------------------- | -------------------------------------------------------------------------------------------- |
| `XUI_NONINTERACTIVE` | 设为 `1`（或在无 TTY 的环境下运行）即可在零提示的情况下完成安装；生成的凭据会写入 `/etc/primevpn/install-result.env`。参见 [安装](/docs/guide/installation#unattended--cloud-init)。 |
