---
title: 多节点与受管主机
description: 通过 API 令牌或 mTLS 信任、心跳检测以及针对每个入站的主机覆盖，从一个主控面板统一管理多个 PRIMEVPN 面板并定制订阅。
icon: Boxes
---

PRIMEVPN 可以从单个主控面板管理**多台服务器**，并借助**受管主机**覆盖每个入站在订阅中的通告方式。

## 节点

**节点**是另一个由你的主控面板通过其 API 管理的 PRIMEVPN 面板。主控会轮询每个节点，并在一处集中展示其状态、版本、CPU/内存、运行时间和流量。
