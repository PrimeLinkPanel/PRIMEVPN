---
title: 备份与恢复
description: 手动或通过 Telegram 机器人备份并恢复你的 PRIMEVPN 数据库与证书。
icon: DatabaseBackup
---
...
- **数据库** —— 默认为位于 `/etc/primevpn/primevpn.db` 的 SQLite（如果你使用
...
`primevpn migrate-db` 命令可在 SQLite 数据库与纯文本 SQL 转储之间相互转换
（便于检查，或在不同机器之间转移）：
