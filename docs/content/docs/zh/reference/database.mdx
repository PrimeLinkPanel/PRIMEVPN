---
title: 数据库
description: PRIMEVPN 的存储后端——SQLite（默认）与 PostgreSQL——涵盖数据库路径、连接池以及从 SQLite 到 PostgreSQL 的迁移。
icon: Database
---

PRIMEVPN 将所有内容——入站、客户端、设置——都存储在数据库中。你在安装时
选择后端；两者都是一等支持。

## SQLite（默认）

位于 `/etc/primevpn/primevpn.db` 的单个文件。无需任何配置，适合中小型
部署。该文件夹可通过
...
选择后端，安装程序会将这些变量写入 `/etc/default/primevpn`：
