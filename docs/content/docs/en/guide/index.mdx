...
**PRIMEVPN** is a web control panel that sits on top of
...
<Mermaid
  chart={`
flowchart LR
  Admin["Admin browser"] -->|HTTPS panel| Panel["PRIMEVPN panel"]
...
- Operational tooling: **multi-node** management, a **Telegram bot**, backups, custom-core IP limiting, and a documented REST API.
...
The default SQLite database lives at `/etc/primevpn/primevpn.db`, and the panel listens
...
PRIMEVPN is aimed at anyone running their own Xray server: from a single personal
...
<Callout type="info">
  PRIMEVPN is an enhanced fork of the original PRIMEVPN project, adding broader protocol
