---
title: First Login
description: Find your generated PRIMEVPN credentials, reach the panel, enable two-factor auth, and harden it before exposing anything.
icon: KeyRound
---
...
`docker exec -it <container> primevpn setting -show`.
...
PRIMEVPN supports TOTP two-factor auth (compatible with Google Authenticator, Aegis,
...
step or with `primevpn setting -resetTwoFactor`.
...
Use a valid certificate (via the `primevpn` menu's SSL management, or a reverse
