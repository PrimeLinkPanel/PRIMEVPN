---
title: گواهی‌های SSL
description: دریافت و تمدید گواهی‌های TLS برای پنل و ورودی‌های PRIMEVPN — با منوی ACME در primevpn (دامنه یا IP خام)، گواهی wildcard از طریق Cloudflare DNS-01، یا Certbot دستی.
icon: ShieldCheck
---
...
- **منوی `primevpn`** — کلاینت داخلی [ACME](https://en.wikipedia.org/wiki/Automatic_Certificate_Management_Environment).
...
## منوی SSL در `primevpn` (Let's Encrypt)

`primevpn` را اجرا کنید و **`20` — SSL Certificate Management** را انتخاب کنید. این گزینه
...
`primevpn` ← `20` ← **Get SSL (Domain)**، سپس دامنه را وارد کنید. acme.sh گواهی را
...
`primevpn` ← **`21` — Cloudflare SSL Certificate**. هنگام پرسش، **`t`** را برای یک
