...
// Fixtures are real lines captured from `journalctl -u primevpn` on a production
...
describe('parseLogLine — SysLog (journalctl) formats', () => {
  it('primevpn go-logging line: keeps level, strips prefix, tags PRIMEVPN', () => {
    const r = parseLogLine(
      'Jun 08 23:57:28 ubuntu-4gb-fsn1-1 /usr/local/primevpn/primevpn[72297]: INFO - mtproto: started mtg for inbound 3 on 0.0.0.0:8443',
    );
    expect(r.stamp).toBe('Jun 08 23:57:28');
    expect(r.levelText).toBe('INFO');
    expect(r.service).toBe('PRIMEVPN:');
    expect(r.body).toBe('mtproto: started mtg for inbound 3 on 0.0.0.0:8443');
  });

  it('xray go-logging line: lifts the XRAY service tag', () => {
    const r = parseLogLine(
      'Jun 08 23:56:52 ubuntu-4gb-fsn1-1 /usr/local/primevpn/primevpn[72297]: WARNING - XRAY: core: Xray 26.6.1 started',
    );
  ...
  it('Go std-log line: strips the redundant embedded date, keeps the message', () => {
    const r = parseLogLine(
      'Jun 08 19:22:22 ubuntu-4gb-fsn1-1 primevpn[1439]: 2026/06/08 19:22:22 http: TLS handshake error from 18.97.5.1:36022: EOF',
    );
  ...
  it('telego bracketed line: lifts the ERROR level out of "[ts] ERROR ..."', () => {
    const r = parseLogLine(
      'Jun 09 00:14:52 ubuntu-4gb-fsn1-1 primevpn[72297]: [Tue Jun  9 00:14:52 UTC 2026] ERROR Retrying getting updates in 8s...',
    );
  ...
  it('systemd line: shows the body rather than a bare timestamp', () => {
    const r = parseLogLine(
      'Jun 08 23:56:47 ubuntu-4gb-fsn1-1 systemd[1]: Stopping primevpn.service - primevpn Service...',
    );
    expect(r.stamp).toBe('Jun 08 23:56:47');
    expect(r.body).toBe('Stopping primevpn.service - primevpn Service...');
  });

  it('never collapses a journald entry to just its timestamp', () => {
    const r = parseLogLine(
      'Jun 09 00:15:00 ubuntu-4gb-fsn1-1 primevpn[72297]: [Tue Jun  9 00:15:00 UTC 2026] ERROR Getting updates: telego: getUpdates: api: 409 "Conflict"',
    );
