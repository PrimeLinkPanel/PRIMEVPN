...
func TestPgRestoreReadFailureError(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			localVersion: "16.4",
			want:         "This backup was created by pg_dump from PostgreSQL 17 or newer, but the server's pg_restore is version 16.4 and cannot read it; run 'primevpn pgclient 17' on the server (or upgrade the postgresql-client package to version 17 or newer), then retry the import",
		},
		{
			name:         "dump from postgres 16 on older client",
			probeOutput:  "pg_restore: error: unsupported version (1.15) in file header",
			localVersion: "15.8",
			want:         "This backup was created by pg_dump from PostgreSQL 16 or newer, but the server's pg_restore is version 15.8 and cannot read it; run 'primevpn pgclient 16' on the server (or upgrade the postgresql-client package to version 16 or newer), then retry the import",
		},
		{
		...
		},
		{
			...
			probeOutput:  "pg_restore: error: unsupported version (1.16) in file header",
			localVersion: "",
			want:         "This backup was created by pg_dump from PostgreSQL 17 or newer, but the server's pg_restore is version unknown and cannot read it; run 'primevpn pgclient 17' on the server (or upgrade the postgresql-client package to version 17 or newer), then retry the import",
		},
		{
