---
title: 首次登录
description: 找到自动生成的 PRIMEVPN 登录凭据，进入面板，启用双因素认证，并在对外暴露任何内容之前完成安全加固。
icon: KeyRound
---
...
```bash title="/etc/primevpn/install-result.env (mode 600)"
XUI_USERNAME=...
XUI_PASSWORD=...
XUI_PANEL_PORT=...
XUI_WEB_BASE_PATH=...
...
```bash
primevpn              # menu → 11 (View Current Settings)
primevpn settings     # or the one-shot form
```

对于 **Docker**，请从容器日志中读取生成的凭据，或运行 `docker exec -it <container> primevpn setting -show`。
...
使用非默认端口和一个长而随机的 **Web 根路径**，能让面板更难被发现。你可以在 UI 的**面板设置**中修改，或通过 `primevpn` 菜单修改：
...
PRIMEVPN 支持 TOTP 双因素认证（兼容 Google Authenticator、Aegis 等）。在**面板设置**中启用即可——启用后，登录页面除了密码外还会要求输入一个 6 位验证码，并且开启该功能会强制所有人重新登录。你可以通过菜单的 **Reset Username & Password** 步骤，或使用 `primevpn setting -resetTwoFactor` 来禁用它。
...
使用有效的证书（通过 `primevpn` 菜单的 SSL 管理，或通过反向代理），使面板仅能通过 HTTPS 访问。
</Step>
