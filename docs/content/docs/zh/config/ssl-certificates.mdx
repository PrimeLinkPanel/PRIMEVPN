---
title: SSL 证书
description: 为 PRIMEVPN 面板和入站获取并续期 TLS 证书——可使用 primevpn 的 ACME 菜单（域名或裸 IP）、Cloudflare DNS-01 通配符，或手动 Certbot。
icon: ShieldCheck
---
...
- **`primevpn` 菜单**——内置的 [ACME](https://en.wikipedia.org/wiki/Automatic_Certificate_Management_Environment)
...
## `primevpn` SSL 菜单（Let's Encrypt）

运行 `primevpn` 并选择 **`20` — SSL Certificate Management**。它会调用
...
`primevpn` → `20` → **Get SSL (Domain)**，然后输入域名。acme.sh 会请求
...
`primevpn` → **`21` — Cloudflare SSL Certificate**。当提示时，选择 **`t`** 使用
