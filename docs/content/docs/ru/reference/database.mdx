---
title: База данных
description: Хранилища данных PRIMEVPN — SQLite (по умолчанию) и PostgreSQL — путь к базе данных, пул соединений и миграция с SQLite на PostgreSQL.
icon: Database
---

PRIMEVPN хранит всё — входящие подключения, клиентов, настройки — в базе данных.
Бэкенд выбирается во время установки; оба являются полноценными.

## SQLite (по умолчанию)

Один файл по пути `/etc/primevpn/primevpn.db`. Никакой настройки не требуется, идеально
...
переменные окружения, которые инсталлятор записывает в `/etc/default/primevpn`:
