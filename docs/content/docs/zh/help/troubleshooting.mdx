---
title: 故障排查
description: 解决 PRIMEVPN 常见问题——面板无法启动、502 错误、证书问题，以及客户端无法连接。
icon: Wrench
---
...
- 检查服务状态和日志（`primevpn` 菜单 → 状态/日志）。
- 确认**面板端口未被**其他服务占用。
- 验证数据库路径可写（默认为 `/etc/primevpn/primevpn.db`）。
...
<Callout type="info">
  仍然无法解决？搜索
  [GitHub issues](https://github.com/sh7CBAC/PRIMEVPN/issues)——你遇到的现象
  很可能此前已有人遇到过。
</Callout>
