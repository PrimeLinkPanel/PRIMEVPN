...
description: Back up and restore your PRIMEVPN database and certificates, manually or via the Telegram bot.
icon: DatabaseBackup
---
...
- **The database** — SQLite at `/etc/primevpn/primevpn.db` by default (or your
...
cp /etc/primevpn/primevpn.db /root/primevpn-backup-$(date +%F).db
```
...
The `primevpn migrate-db` command converts the SQLite database to and from a plain
SQL text dump (handy for inspection or transferring between machines):

```bash
primevpn migrate-db --dump /root/primevpn.sql       # SQLite -> SQL text
primevpn migrate-db --restore /root/primevpn.sql     # SQL text -> SQLite
```
