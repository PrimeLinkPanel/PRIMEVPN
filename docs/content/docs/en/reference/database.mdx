---
title: Database
description: PRIMEVPN storage backends — SQLite (default) and PostgreSQL — the database path, connection pool, and SQLite-to-PostgreSQL migration.
icon: Database
---

PRIMEVPN stores everything — inbounds, clients, settings — in a database. You choose
the backend at install time; both are first-class.

## SQLite (default)

A single file at `/etc/primevpn/primevpn.db`. Zero setup, ideal for small and medium
...
writes to `/etc/default/primevpn`:
...
Then set `XUI_DB_TYPE` and `XUI_DB_DSN` in `/etc/default/primevpn` and restart:
