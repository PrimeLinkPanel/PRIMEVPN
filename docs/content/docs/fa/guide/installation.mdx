---
title: نصب
description: نصب PRIMEVPN با اسکریپت رسمی (نسخهٔ پایدار، نسخهٔ مشخص یا dev-latest)، نصب خودکار/cloud-init یا Docker — و انتخاب میان SQLite و PostgreSQL.
icon: Download
---

‏PRIMEVPN روی طیف گسترده‌ای از توزیع‌های Linux اجرا می‌شود — Ubuntu، Debian، Armbian،
...
<Callout type="warn">
  ...
  سرویس نصب می‌کند، دستور مدیریتی `primevpn` را راه‌اندازی می‌کند و پنل را در زمان
  بوت فعال می‌سازد.
</Callout>
...
دستور مدیریتی `primevpn` را نصب می‌کند.
...
پس از پایان نصب، مشخصات ورود چاپ‌شده را یادداشت کنید و دستور `primevpn` را اجرا کنید تا
[منوی مدیریت](/docs/guide/update-uninstall#the-primevpn-management-menu) باز شود، سپس
...
ترجیح می‌دهید از ایمیج ازپیش‌ساخته استفاده کنید؟ این ایمیج روی GitHub Container Registry منتشر شده است:
...
والیوم `db/` پایگاه‌دادهٔ SQLite (`/etc/primevpn/primevpn.db`) را نگه می‌دارد و `cert/`
...
[صفحهٔ releases](https://github.com/sh7CBAC/PRIMEVPN/releases) دانلود کنید، آن را
...
- **SQLite** (پیش‌فرض) — یک فایل واحد در مسیر `/etc/primevpn/primevpn.db`. بدون نیاز به هیچ تنظیماتی.
...
`/etc/primevpn/install-result.env` بنویسد:

```bash
XUI_NONINTERACTIVE=1 bash <(curl -Ls https://raw.githubusercontent.com/sh7CBAC/PRIMEVPN/main/install.sh)
```

دایرکتوری [`deploy/`](https://github.com/sh7CBAC/PRIMEVPN/tree/main/deploy) در مخزن
...
<Cards>
  ...
  <Card
    ...
    description="منوی primevpn، به‌روزرسانی‌ها و حذف."
  />
  <Card
    title="REALITY"
