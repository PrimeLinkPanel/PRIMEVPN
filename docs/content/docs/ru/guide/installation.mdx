---
title: Установка
description: Установите PRIMEVPN с помощью официального скрипта (стабильная, фиксированная или dev-сборка), в автоматическом режиме / через cloud-init либо в Docker — и выберите SQLite или PostgreSQL.
icon: Download
---

PRIMEVPN работает на широком спектре дистрибутивов Linux — Ubuntu, Debian, Armbian,
...
<Callout type="warn">
  ...
  устанавливает службу, настраивает команду управления `primevpn` и включает
...
настраивает службу и устанавливает команду управления `primevpn`.
...
bash <(curl -Ls https://raw.githubusercontent.com/sh7CBAC/PRIMEVPN/main/install.sh) v3.4.0
```
...
По завершении запишите выведенные данные для входа и выполните `primevpn`, чтобы
открыть [меню управления](/docs/guide/update-uninstall#the-primevpn-management-menu),
...
Предпочитаете готовый образ? Он опубликован в GitHub Container Registry:

```bash title="docker run"
docker run -d \
  -v $PWD/db/:/etc/primevpn/ \
  -v $PWD/cert/:/root/cert/ \
  --network=host \
  --restart=unless-stopped \
  --name PRIMEVPN \
  ghcr.io/sh7cbac/primevpn:latest
```

Том `db/` хранит базу данных SQLite (`/etc/primevpn/primevpn.db`), а `cert/` — TLS-сертификаты,
...
[страницы релизов](https://github.com/sh7CBAC/PRIMEVPN/releases), распаковать его и
...
- **SQLite** (по умолчанию) — единственный файл по пути `/etc/primevpn/primevpn.db`. Без
...
записав их в `/etc/primevpn/install-result.env`:

```bash
XUI_NONINTERACTIVE=1 bash <(curl -Ls https://raw.githubusercontent.com/sh7CBAC/PRIMEVPN/main/install.sh)
```

В каталоге [`deploy/`](https://github.com/sh7CBAC/PRIMEVPN/tree/main/deploy)
...
<Cards>
  ...
  <Card
    ...
    description="Меню primevpn, обновления и удаление."
  />
  <Card
    title="REALITY"
