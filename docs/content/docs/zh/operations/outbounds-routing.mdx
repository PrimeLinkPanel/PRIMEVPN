---
title: 出站与路由
description: 在 PRIMEVPN 中调整出口流量——WARP 与 NordVPN 出站、出站订阅（服务器池）、路由规则以及负载均衡器。
icon: Route
---

入站负责接受客户端；**出站**则决定客户端的流量接下来发往何处。
PRIMEVPN 可以让流量经由 Cloudflare WARP、NordVPN，或从订阅导入的任意出站池转发，
...
服务器——便于进行链式代理或将部分流量发往境外。注意 PRIMEVPN 所使用的各种线路
...
WARP 让你的服务器经由 Cloudflare 的网络出口。PRIMEVPN 可以为你注册一个
...
在面板的 WARP 控制项中注册一个账户。PRIMEVPN 会自动填入该出站的
密钥、地址、保留字节以及对端端点。
</Step>
...
PRIMEVPN 可以根据访问令牌获取 NordVPN（NordLynx/WireGuard）凭据（或
