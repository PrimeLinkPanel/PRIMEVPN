...
- 运维工具：**多节点**管理、**Telegram 机器人**、备份、由 PRIMEVPN 自定义核心执行的 IP 限制，以及一套有文档说明的 REST API。
...
默认的 SQLite 数据库位于 `/etc/primevpn/primevpn.db`，面板默认监听 **2053** 端口。两者均可配置——参见
[首次登录](/docs/guide/first-login)和环境变量参考。

## 它适合谁

PRIMEVPN 面向任何运行自己 Xray 服务器的人：从一台个人 VPS，到管理众多节点和客户端的运营者。如果你想拥有 Xray-core 的强大能力，又不想终日埋首于 JSON 配置文件，那么它正适合你。

<Callout type="info">
  PRIMEVPN 是原始 PRIMEVPN 项目的增强分支，增加了更广泛的协议支持、更高的稳定性、按客户端的流量统计、多节点管理，以及许多提升使用体验的特性。
</Callout>
