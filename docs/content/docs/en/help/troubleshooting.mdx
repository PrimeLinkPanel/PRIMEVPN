...
description: Fix common PRIMEVPN problems — the panel won't start, 502 errors, certificate issues, and clients that can't connect.
icon: Wrench
---
...
- Check the service status and logs (`primevpn` menu → status/logs).
- Make sure the **panel port isn't already in use** by another service.
- Verify the database path is writable (default `/etc/primevpn/primevpn.db`).
...
<Callout type="info">
  ...
  [GitHub issues](https://github.com/sh7CBAC/PRIMEVPN/issues) — your symptom has
