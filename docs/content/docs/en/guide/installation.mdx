---
title: Installation
description: Install PRIMEVPN via the official script (stable, pinned, or dev-latest), unattended/cloud-init, or Docker — and choose SQLite or PostgreSQL.
icon: Download
---

PRIMEVPN runs on a wide range of Linux distributions — Ubuntu, Debian, Armbian,
...
<Callout type="warn">
  ...
  sets up the `primevpn` management command, and enables the panel on boot.
</Callout>
...
and installs the `primevpn` management command.
...
When it finishes, note the printed login details and run `primevpn` to open the
[management menu](/docs/guide/update-uninstall#the-primevpn-management-menu), then
...
Prefer the prebuilt image? It's published to the GitHub Container Registry:
...
The `db/` volume holds the SQLite database (`/etc/primevpn/primevpn.db`) and `cert/`
...
[releases page](https://github.com/sh7CBAC/PRIMEVPN/releases), extract it, and run
...
- **SQLite** (default) — a single file at `/etc/primevpn/primevpn.db`. Zero setup.
...
`/etc/primevpn/install-result.env`:

```bash
XUI_NONINTERACTIVE=1 bash <(curl -Ls https://raw.githubusercontent.com/sh7CBAC/PRIMEVPN/main/install.sh)
```

The repo's [`deploy/`](https://github.com/sh7CBAC/PRIMEVPN/tree/main/deploy)
...
<Cards>
  ...
  <Card
    ...
    description="The primevpn menu, updates, and removal."
  />
  <Card
    title="REALITY"
