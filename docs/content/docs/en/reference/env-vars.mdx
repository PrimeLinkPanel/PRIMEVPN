...
description: Complete reference for PRIMEVPN's XUI_* environment variables — database, panel, logging, memory, and the tunnel health monitor.
icon: Variable
---

PRIMEVPN reads its runtime configuration from `XUI_*` environment variables. On a
script install the installer writes them to the service environment file
(`/etc/default/primevpn`, or `/etc/conf.d/primevpn` / `/etc/sysconfig/primevpn` depending on
...
| `XUI_DB_FOLDER`          | `/etc/primevpn` | Folder for the SQLite database file (`primevpn.db`).                     |
...
The default SQLite database path is `/etc/primevpn/primevpn.db`. See
...
| `XUI_LOG_FOLDER` | `/var/log/primevpn`  | Log output directory.                                       |
...
| -------------------- | -------------------------------------------------------------------------------------------- |
| `XUI_NONINTERACTIVE` | Set to `1` (or run with no TTY) to install with zero prompts; generated credentials are written to `/etc/primevpn/install-result.env`. See [Installation](/docs/guide/installation#unattended--cloud-init). |
